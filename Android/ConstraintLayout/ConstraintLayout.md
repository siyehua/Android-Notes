...
近日伴随着Android Studio 2.2的正式推送,最大的亮点是布局的蓝图模式以及与之配套的ConstraintLayout.

ConstraintLayout,翻译过来,可以叫约束布局,其子view就是通过一个个属性的约束,来决定自己的位置,大小,而传统的RelativeLayout也类似,所以可以看成是RelativeLayout的一种进化版版本,属性用法相对来说RelativeLayout较为复杂,但是当你熟悉之后你会爱上它的.
...
而上面的属性和RelativeLayout基本上是一致的,值可是是任意id与parent.parent是表示相对于ConstraintLayout.
...
###约束属性值为parent时对照表
当值为parent时,必须成对出现才有意义,即top与bottom,left与right,start与end成对
以前使用RelativeLayout的时候,假设子View设置属性
...
鼠标拖动小球,上下移动会发现数值,控件垂直方向上的位置都跟着改变.拖动到20,切换到代码,新增了一个属性,此时控件的上下比例是:0.8:0.2
...
```
该属性在1.0.0-alpha8暂时未有任何作用,查看源码发现,ConstraintLayout只是对改属性值进行了接受,但是没有做任何处理,应该在后续版本会新增其功能
...
而使用ConstraintLayout则还是不能做到这点,因为前面对于其介绍的比例,是相对parent来说的,如果非常均分,可以可是借助透明的分割线做到这点
...
app:layout_constraintGuide_percent="0.333"//决定当航线的位置
/>
 ```
